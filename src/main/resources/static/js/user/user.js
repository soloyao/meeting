$(function() {
	...
	var data4Vue = {
		roles: [],
		users: [],
		user4Add: {id: 0, name: "", realname: "", password: "", number: "", email: "", job: "", deptName: "", roles: []},
		pagination: {},
		keyword: "",
		isEditShow: false,
		isLoading: false,
	...
	var vue = new Vue({
		...
		methods: {
			listDepts() {
				...
				axios.get(url).then(function(res) {
					res.data.map(function(item) {
						$("#deptName").append("<option value='" + item.id + "'>" + item.name + "");
					});
				});
			},
			...
			save: function() {
				var _this = this;
				console.log(this.user4Add);
				if (!this.user4Add.name || !this.user4Add.realname || !this.user4Add.password || !this.user4Add.number || !this.user4Add.email || !this.user4Add.job || !this.user4Add.deptId) {
				...
				if (_this.user4Add.id == 0) { //add
					axios.post(url, _this.user4Add).then(function(res) {
						if (res.data.code == 0) {
							_this.list(1);
							_this.user4Add = {id: 0, name: "", realname: "", password: "", number: "", email: "", job: "", deptName: "", roles: []};
			...
			addEdit: function() {
				...
				this.editTitle = "新增";
				this.user4Add = {id: 0, name: "", realname: "", password: "", number: "", email: "", job: "", deptName: "", roles: []};
			...
			updateEdit: function(user) {
				...
				this.user4Add.number = user.number;
				this.user4Add.email = user.email;
				this.user4Add.job = user.job;
				this.user4Add.deptId = user.deptId;
				this.user4Add.deptName = user.deptName;
			...
			add: function() {
				var _this = this;
				var url = "users";
				if (!this.user4Add.name || !this.user4Add.realname || !this.user4Add.password || !this.user4Add.number || !this.user4Add.email || !this.user4Add.job || !this.user4Add.deptName) {
					myzui._error("必填参数不能为空");
					return;
				}
